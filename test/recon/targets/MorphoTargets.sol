    ...
    Properties
{
    ...
    //     morpho_flashLoan(_getAsset(), assets);
    // }
    ...
    function morpho_repay_clamped_assets(uint256 assets) public {
        // address onBehalf = _getActorThenSwitchActor(senderEntropy); 
    ...
    // function morpho_setAuthorization_clamped(uint256 senderEntropy, bool newIsAuthorized) public {
    //     // address authorized = _getActorThenSwitchActor(senderEntropy);
    ...
    // function morpho_withdrawCollateral_clamped(uint256 senderEntropy, uint256 assets, address receiver) public {
    //     address onBehalf = _getActorThenSwitchActor(senderEntropy);
    ...
    function morpho_flashLoan(address token, uint256 assets) public asFlashLoanCallback {
        MockERC20(token).approve(address(morpho), assets);

        morpho.flashLoan(token, assets, "");
    }

    function morpho_liquidate(address borrower, uint256 seizedAssets, uint256 repaidShares) public asActor {
        morpho.liquidate(activeMarketParams, borrower, seizedAssets, repaidShares, "");
    }

    function morpho_repay(uint256 assets, uint256 shares, address onBehalf) public asActor {
    ...
    function morpho_supply(uint256 assets, uint256 shares, address onBehalf) public asActor {
        morpho.supply(activeMarketParams, assets, shares, onBehalf, "");
    }

    function morpho_supplyCollateral(uint256 assets, address onBehalf) public asActor {
    ...
    function morpho_withdrawCollateral(uint256 assets, address onBehalf, address receiver) public asActor {
        morpho.withdrawCollateral(activeMarketParams, assets, onBehalf, receiver);
    }
}